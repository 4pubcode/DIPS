...
int hPps::init(vector<CountType> elements,
...
// class hybridSubsetSampling : public subsetSamplingMethod {
//    private:
//     CountType n;
//     pair<CountType, WeightType> *nameProb, *X, *Y;
//     double pm, sum, p_mu, p_thres;
...
//    public:
//     hybridSubsetSampling(/* args */);
//     ~hybridSubsetSampling();
//     int init(vector<CountType> elements, vector<WeightType> weights);
//     int query(vector<CountType> &results);
//     int add(WeightType addWeight, CountType idx);
//     int del(CountType delIdx);
...
// hybridSubsetSampling::hybridSubsetSampling(/* args */) {}

// hybridSubsetSampling::~hybridSubsetSampling() {}

// int hybridSubsetSampling::query(vector<CountType> &results) {
//     mt19937 generator(time(NULL));
//     results.resize(0);
//     if (n == 0) return 0;
//     for (CountType i = 0; i < Ysize; i++) {
//         if (dsfmt_gv_genrand_open_close() < MY_C * Y[i].second) {
//             if (complement == 0) results.push_back(Y[i].first);
//         } else {
//             if (complement == 1) results.push_back(Y[i].first);
//         }
...
//         if (idx >= nX - i) idx = nX - i - 1;
//         swap(X[idx + i], X[i]);
//         if (dsfmt_gv_genrand_open_close() < MY_C * X[i].second / pm) {
//             if (complement == 0) results.push_back(X[i].first);
//         } else {
//             if (complement == 1) results.push_back(X[i].first);
//         }
...
// int hybridSubsetSampling::init(vector<CountType> elements, vector<WeightType>
// weights) {
//     n = weights.size();
//     if (n == 0) return 0;
//     if (n > CountTypeMAX / 2)
...
// int hybridSubsetSampling::_add(WeightType addProb, CountType idx) {
//     n++;
//     if (n > newSize) {
//         cout << "error, overflow" << endl;
//         exit(-1);
...
// int hybridSubsetSampling::_del(CountType delIdx) {
//     n--;
//     sum -= nameProb[dict[delIdx]].second;
//     nameProb[dict[delIdx]] = nameProb[n];
//     dict[nameProb[n].first] = dict[delIdx];
...
// int hybridSubsetSampling::add(WeightType addWeight, CountType idx) {
//     vector<CountType> elements(n+1);
//     vector<WeightType> weights(n+1);
//     for (int i=0; i<n; i++) {
//         elements[i] = nameProb[i].first;
...
// int hybridSubsetSampling::del(CountType delIdx) {
//     vector<CountType> elements(n-1);
//     vector<WeightType> weights(n-1);
//     int last = 0;
//     for (int i=0; i<n; i++) {
