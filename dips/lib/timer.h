...
class Timer {
   private:
    std::chrono::high_resolution_clock::time_point __StartTime, __LastTime,
        __EndTime;
   ...
   public:
    ...
    void log_operation_time(const char *operationName) {
        const double duration = this->get_operation_time();
        std::cout << "->Time used (sec) for operation [" << operationName
                  << "]: " << duration << '\n';
    }
    ...
    void log_total_time() {
        ...
        std::cout << "--->Time used (sec) for process [" << __processName
                  << "]: " << duration << '\n';
    }
