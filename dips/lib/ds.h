...
void my_index::mysample(vector<CountType> &results) {
    ...
    if (elements.size() <= 8) {
        for (int i = 0; i < elements.size(); i++) {
            if (dsfmt_gv_genrand_open_close() * W <= c * weights[i]) {
...
void my_index::sample(vector<CountType> &results) {
    ...
    for (int i = r; i >= r - 2 && Ii2itv.find(i) != Ii2itv.end(); --i) {
        ...
        // }
        vector<CountType> temp_results;
        if (itv.m_idx == nullptr) {
            Ri2r[itv.ris[0]].sample(temp_results, true);
        } else {
...
void my_index::_del(CountType e, WeightType w) {
    ...
    if (e2ind.find(e) == e2ind.end()) {
        cout << "Deleting non-existing element!" << endl;
    }
    auto ind_e = e2ind[e];
    W -= w;
    if (W < 0) {
...
void interval::del(int Ri, WeightType w) {
    ...
    // Ii
    if (Ri2ind.find(Ri) == Ri2ind.end()) {
        cout << "Deleting non-existing range!" << endl;
    }
    auto ind_ri = Ri2ind[Ri];
