...
static inline void MakeMaxHeap(vector<pair<double, uint32_t>> &vec)
{
    // Max heap
    const auto size = vec.size();
...
/// Replace the value for the first element and down-heap this element.
static inline void MaxHeapReplaceMaxValue(vector<pair<double, uint32_t>> &vec, const double &val)
{
    // Increase the value of the first element
    const auto size = vec.size();
    size_t i, child;
...
static inline size_t GenRandomNodeByWeightLT(const Edgelist &edges)
{
    ...
    if (weight < edges.front().second)
        return 0; // First element

    if (weight > edges.back().second)
        return edges.size() + 1; // No element
...
bool SmallerPair(const pair<uint32_t, uint32_t> &x, const pair<uint32_t, uint32_t> &y)
{
    return x.first < y.first;
}

bool GreaterPair(const pair<uint32_t, uint32_t> &x, const pair<uint32_t, uint32_t> &y)
{
    return x.first > y.first;
}
