...
class Alias {
   public:
    ...
    int n;
    Alias() {}
    void input(pair<unsigned int, double>* pi, uint size) {
        n = size;
        double sum = 0;
        // stack<int> small;
        // stack<int> big;
    ...
    int generateRandom(const double& rand1, const double& rand2) {
        int firstId = rand1 * n;
        int answer = rand2 < p[firstId] ? map1[firstId] : map1[h[firstId]];
        return answer;
    }
};
